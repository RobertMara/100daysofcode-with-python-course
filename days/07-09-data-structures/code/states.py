# From the day N+2 Bite https://codechalleng.es/bites/promo/listvsdict mentioned in README.md.
# This is Bite 89, which is not in the Challenges project.
...
def get_every_nth_state(states=states, n=10):
    ...
    # Of course, this does not test for values out of range....
    tenths = [states[i] for i in range(n-1, len(states), n)]
    return tenths
...
def get_longest_state(data):
    """Receives data, which can be the us_state_abbrev dict or the states
       ...
       of the string"""
   # state_len_hash = [{state: len(state)} for state in data]
   # for (state, length) in state_len_hash.items():

    #sorted() returns dict keys as a sorted list.
    #So whether we use sorted() on states or us_state_abbrev
    #we end up with a sorted list of state names.
...
def combine_state_names_and_abbreviations(us_state_abbrev=us_state_abbrev,
    ...
    new_list = states[-10:]
    new_list.extend( list(us_state_abbrev.values())[:10] )
    new_list.sort
    return new_list
